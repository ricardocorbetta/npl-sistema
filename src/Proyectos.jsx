...
const api = {
    async list() {
          const { data, error } = await supabase.from("proyectos").select("*").order("numero_proyecto", { ascending: true, nullsFirst: false });
...
function ProyectoCard({ p, onEdit, onDelete }) {
    return (
          <div onClick={() => onEdit(p)} style={{ background: "#fff", border: "1px solid #e8e8e8", borderRadius: 10, padding: "11px 13px", marginBottom: 8, cursor: "pointer", transition: "box-shadow .15s" }}
