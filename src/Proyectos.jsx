...
const ESTADOS = [
  ...
  { id: "revision",  label: "Revisión",  color: "#fff8e1", text: "#f57f17" },
];
...
const CHECKS = [
  ...
  { key: "check_diagnostico",label: "Diagnóstico", abbr: "D" },
    ...
    <div style={{ display: "flex", gap: 3, alignItems: "center" }}>
      {CHECKS.map(c => (
        <span key={c.key} title={c.label} style={{
          ...
          background: p[c.key] ? "#111" : "#efefef",
          color: p[c.key] ? "#fff" : "#bbb",
...
function ProyectoCard({ p, onEdit, onDelete }) {
  return (
    <div onClick={() => onEdit(p)}
      style={{ background: "#fff", border: "1px solid #e8e8e8", borderRadius: 10, padding: "11px 13px", marginBottom: 8, cursor: "pointer", transition: "box-shadow .15s" }}
      onMouseEnter={e => e.currentTarget.style.boxShadow = "0 3px 10px rgba(0,0,0,.09)"}
      onMouseLeave={e => e.currentTarget.style.boxShadow = "none"}
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 6, marginBottom: 4 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          {p.numero_proyecto && <span style={{ fontSize: 11, color: "#bbb", fontWeight: 600 }}>{p.numero_proyecto} B��</span>}
          <span style={{ fontSize: 13, fontWeight: 600, color: "#111" }}>{p.descripcion}</span>
        </div>
        <button onClick={e => { e.stopPropagation(); onDelete(p.id); }}
          style={{ background: "none", border: "none", cursor: "pointer", color: "#d0d0d0", fontSize: 17, padding: 0, lineHeight: 1 }}>×</button>
      </div>
...
function Kanban({ items, onEdit, onDelete }) {
  const COLS = [
    ...
    { id: "revision", label: "⚢ Revisión", bg: "#fffcf0", filter: p => p.estado === "revision" },
        ...
        return (
          <div key={col.id} style={{ flex: "0 0 268px", background: col.bg, borderRadius: 12, padding: 12, border: "1px solid #efefef" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              ...
              <span style={{ fontSize: 11, color: "#888", background: "#e8e8e8", borderRadius: 12, padding: "1px 8px", fontWeight: 600 }}>{lcolItems.length}</span>
            </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
      {items.map(p => (
        <div key={p.id} onClick={() => onEdit(p)}
          ...
          <CheckProgress p={p} />
          <button onClick={e => { e.stopPropagation(); onDelete(p.id); }} style={{ background: "none", border: "none", cursor: "pointer", color: "#ccc", fontSize: 16, marginLeft: "auto" }}>×</button>
        </div>
      ))}
    </div>
  );
...
function Form({ item, onSave, onCancel }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (item?.id) await api.update(item.id, data);
  ...
  return (
    <div style={{ ...s.sans, maxWidth: 620, margin: "0 auto", padding: 24 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        <div><label style={s.label}>Número</label><input style={s.input} value={form.numero_proyecto || ""} onChange={e => set("numero_proyecto", e.target.value)} placeholder="ej: 539" /></div>
        <div><label style={s.label}>Estado</label><select style={s.input} value={form.estado} onChange={e => set("estado", e.target.value)}>{EST@DOS.map(e => <option key={e.id} value={e.id}>{e.label}</option>)}</select></div>
        <div style={{ gridColumn: "1 / -1" }}><label style={s.label}>Descripción *</label><input style={s.input} value={form.descripcion} onChange={e => set("descripcion", e.target.value)} placeholder="Ej: Vivienda Abasto Ejecución Steel" /></div>
        ...
        <div><label style={s.label}>Categoría</label><select style={s.input} value={form.categoria || "proyecto"} onChange={e => set("categoria", e.target.value)}>{CATEGORIAS.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
        <div style={{ gridColumn: "1 / -1" }}><label style={s.label}>Pr̳zXim̃ tarea</label><input style={s.input} value={form.proxima_tarea || ""} onChange={e => set("proxima_tarea", e.target.value)} placeholder="Ej: Entregar — 8 de Junio" /></div>
      </div>
...
export default function Proyectos() {
  ...
  const filtrados = items.filter(p => {
    ...
    const okB = !q || [p.descripcion, p.cliente, p.numero_proyecto, p.encargado, p.proxima_tarea].some(v => v?.toLowerCase().includes(q));
  ...
  const kpis = { activos: items.filter(p => p.estado === "activo").length, revision: items.filter(p => p.estado === "revision").length, entregados: items.filter(p => p.estado === "entregado").length, sinCobrar: items.filter(p => p.entregado && !p.cobrado).length };
  if (editando !== null) return <div style={s.sans}><Form item={editando || null} onSave={() => { setEditando(null); load(); }} onCancel={() => setEditando(null)} /></div>;
  return (
    <div style={{ ...s.sans, padding: "16px 20px", maxWidth: 1600, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
        <div><h1>Proyectos</h1><p style={{ fontSize: 13, color: "#888" }}>{items.length} proyectos en total</p></div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(120px,1fr))", gap: 10, marginBottom: 14 }}>
        {[["Activos", kpis.activos, "#fff"], ["Revisión", kpis.revision, "#fffcf0"], ["Entregados", kpis.entregados, "#f5f9ff"], ["Sin cobrar", kpis.sinCobrar, "#fff5f5"]].map(([l, v, bg]) => (
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
        <input placeholder="Buscar proyecto, cliente, número..." value={busq} onChange={e => setBusq(e.target.value)} style={{ ...s.input, flex: 1, minWidth: 200 }} />
        ...
        {(busq || filtroEst !== "todos" || filtroEnc !== "todos") && <button onClick={() => { setBusq(""); setFiltroEst("todos"); setFiltroEnc("todos"); }} style={{ ...s.btn, background: "#f5f5f5", color: "#888", border: "1px solid #e5e5e5", fontSize: 12 }}>Limpiar</button>}
      </div>
